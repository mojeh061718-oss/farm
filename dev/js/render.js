...
const Renderer = (() => {
  ...
  function render(state, dt) {
    ...
    drawHD();                            // HD PREVIEW: definition grade (sharpen + micro-contrast + bloom)
  }

  /* ================= HD PREVIEW post-process =================
     A final full-frame grade that lifts perceived definition without touching
     the world art: a mild unsharp (SVG convolve), a micro-contrast/saturation
     lift, and a soft highlight bloom. Runs on the /dev build only. */
  let hdBuf, hdBufCtx, hdBloom, hdBloomCtx, hdWarned = false;
  // Tap the HD badge to cycle window.__HD_MODE: 0 = off (production look),
  // 1 = grade (saturate/contrast/brightness + highlight bloom), 2 = grade +
  // unsharp (SVG convolve — GPU-fast on-device, CPU-brutal in headless).
  const HD_GRADE = 'saturate(1.13) contrast(1.065) brightness(1.015)';
  function ensureHD() {
    if (!hdBuf) {
      hdBuf = document.createElement('canvas'); hdBufCtx = hdBuf.getContext('2d');
      hdBloom = document.createElement('canvas'); hdBloomCtx = hdBloom.getContext('2d');
    }
    if (hdBuf.width !== canvas.width || hdBuf.height !== canvas.height) {
      hdBuf.width = canvas.width; hdBuf.height = canvas.height;
      hdBloom.width = Math.max(1, canvas.width >> 1); hdBloom.height = Math.max(1, canvas.height >> 1);
    }
  }
  function drawHD() {
    const mode = window.__HD_MODE == null ? 1 : window.__HD_MODE;
    if (!mode) return; // 0 = off: identical to production
    try {
      ensureHD();
      const W = canvas.width, H = canvas.height;
      // snapshot the finished frame
      hdBufCtx.setTransform(1, 0, 0, 1, 0, 0);
      hdBufCtx.clearRect(0, 0, W, H);
      hdBufCtx.drawImage(canvas, 0, 0);
      // highlight bloom: a bright, blurred half-res copy
      hdBloomCtx.setTransform(1, 0, 0, 1, 0, 0);
      hdBloomCtx.clearRect(0, 0, hdBloom.width, hdBloom.height);
      hdBloomCtx.filter = 'brightness(1.28) contrast(1.7) blur(3px)';
      hdBloomCtx.drawImage(hdBuf, 0, 0, hdBloom.width, hdBloom.height);
      hdBloomCtx.filter = 'none';
      // recomposite the graded frame over the canvas (+ unsharp in mode 2)
      ctx.setTransform(1, 0, 0, 1, 0, 0);
      ctx.globalCompositeOperation = 'source-over';
      ctx.filter = (mode === 2 ? 'url(#hd-sharpen) ' : '') + HD_GRADE;
      ctx.drawImage(hdBuf, 0, 0);
      ctx.filter = 'none';
      // screen the bloom back in
      ctx.globalCompositeOperation = 'screen';
      ctx.globalAlpha = 0.14;
      ctx.drawImage(hdBloom, 0, 0, W, H);
      ctx.globalAlpha = 1;
      ctx.globalCompositeOperation = 'source-over';
    } catch (e) {
      if (!hdWarned) { hdWarned = true; console.warn('HD pass disabled:', e); }
    }
  }
