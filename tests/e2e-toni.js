...
(async () => {
  ...
  const off = await page.evaluate(() => {
    ...
    G.buyParcel(1);
    G.till(13, 5);
    G.plant(13, 5, 'tomato'); // off-season warning is expected
    s.tiles[5][13].crop.water = 1;
    for (let i = 0; i < 8; i++) G.tick(1);
    const cin = s.tiles[8][9].crop, cout = s.tiles[5][13].crop;
  ...
  const storm = await page.evaluate(() => {
    ...
    const outside = s.tiles[5][13].crop; // unblessed tomato from above
    let tries = 0;
  ...
  const frost = await page.evaluate(() => {
    ...
    s.tiles[5][13].crop = { id: 'tomato', prog: 0.3, water: 1, wilt: 0, rot: 0, dead: false, fert: false, regrown: false };
    const outside = s.tiles[5][13].crop;
    let tries = 0;
    while (!outside.dead && tries < 400) {
  ...
  const crow = await page.evaluate(() => {
    ...
    s.tiles[5][13].crop = null; // no unblessed targets anywhere
    s.tiles[7][10].k = 'soil';
